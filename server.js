...
app.get("/api/window-stair-line", cors(), (req, res) => {
  lineController
    .queryDataLine("window-stair-line")
    .then((result) => res.send(result));
});
...
app.get("/api/door-roof-line", cors(), (req, res) => {
  lineController
    .queryDataLine("door-line")
    .then((result) => res.send(result));
  // let promise = []
  // for (let i = 0; i < temp.length; i++) {
  //   promise.push(
  //     lineController.createLine(temp[i].properties, temp[i].geometry.coordinates)
  //   )
  // }
  // Promise.all(promise).then(result => res.send(result))
});
